"""This is taken from https://github.com/callummcdougall/path_patching/blob/main/path_patching.py
"""
import torch as t
...
class Node:
    ...
    @property
    def act_name(self):
        # Extract the block number from the activation name using regex
        block_number = re.search(r"blocks\.(\d+)\.attn\.hook_z", self.activation_name).group(1)
        # Return the formatted string using the block number and head attribute
        return f"A{block_number}.{self.head}"
...
def path_patch(
    ...
    if not any([isinstance(receiver_nodes, IterNode), isinstance(sender_nodes, IterNode)]):
        print(f"--- path patch {sender_nodes} -> {receiver_nodes} --> logits ---")
    ...
    if isinstance(receiver_nodes, IterNode):
        ...
        for node_name, node_shape_dict in receiver_nodes.shape_values.items():
            if verbose:
                ...
                print(f"-- path patch {sender_nodes} -> {node_name} -> logits --")
        return {
    ...
    # If we're fixing receiver(s), and iterating over senders:
        ...
        for node_name, node_shape_dict in sender_nodes.shape_values.items():
            if verbose: 
                print(f"results[{node_name!r}].shape = ({', '.join(f'{s}={v}' for s, v in node_shape_dict.items())})")
                print(f"-- path patch {node_name} -> {receiver_nodes} -> logits --")
        return {
